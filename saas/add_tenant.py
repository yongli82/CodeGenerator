    ...
    for filename in files:
        ...
        if os.path.isfile(file_path):
            if filename.endswith(".java"):
                ...
                elif filename.endswith("Data.java"):
                    handle_data_java(file_path, method_list)
                elif filename.endswith("ServiceObject.java"):
                    handle_service_object_java(file_path, method_list)
                elif "/siteweb/" in file_path and (filename.endswith("Service.java") or filename.endswith("Cache.java")) :
...
def handle_data_java(file_path, method_list):
    ...
    content = read_file(file_path)
    package = re.findall(r"package\s+([\w\.]+);", content)[0]
    class_name_list = re.findall(r"public\s+(?:class)\s+(\w+)\s+implements\s+Serializable\s*\{", content)
    if not class_name_list:
        return
    class_name = class_name_list[0]
...
def handle_test_java(file_path, method_list):
    has_modify = False
    content = read_file(file_path)
    if "private String tenant = BudgetConstant.DEFAULT_TENANT_ID" in content:
        return
    else:
        has_modify = True
        content = content.replace("import org.junit.Test;", "import com.dianping.ba.finance.budget.api.constants.BudgetConstant;\nimport org.junit.Test;", 1)
        content = content.replace("Test {", "Test {\n    private String tenant = BudgetConstant.DEFAULT_TENANT_ID;")
...
def handle_service_object_java(file_path, method_list):
    ...
    method_define_list = re.findall(r"((?:(?:public|private|protected)\s+)([\w<>,\s]+)\s+(\w+)\(([^\)]*)\)\s*(?:\{|throws))", content)
    for method_define in method_define_list:
        result_type = method_define[1]
        method_name = method_define[2]
        if method_name.startswith("set") or method_name.startswith("get") or method_name in ["doInTransaction", "run", "compare"]:
            #setter/getter不需要加租户
            continue
        parameter_str = method_define[3]
